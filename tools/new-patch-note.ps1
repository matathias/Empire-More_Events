...
# Run from the Empire-Events/ directory (the one containing About/, 1.6/, etc.)
...
if (-not (Test-Path $defsDir) -or -not (Test-Path $aboutPath)) {
    Write-Host "ERROR: Run this script from the Empire-Events/ directory (the one containing About/ and 1.6/)." -ForegroundColor Red
    exit 1
}
...
$defName = "EV_${major}_${minor}_${patch}"
...
${indent}<FactionColonies.PatchNoteDef ParentName="EVPatchBase">
...
$csprojPath = "1.6\Source\Core\Empire.Events.csproj"
if (Test-Path $csprojPath) {
    ...
    Write-Host "  Empire.Events.csproj AssemblyVersion/FileVersion updated to $assemblyVersion" -ForegroundColor Cyan
} else {
